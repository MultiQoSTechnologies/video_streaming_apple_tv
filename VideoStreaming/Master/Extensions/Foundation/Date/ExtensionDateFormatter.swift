...
extension DateFormatter {
    ...
    static func dateStringFrom(timestamp: Double?, withFormate:String?) -> String {
        let fromDate:Date = Date(timeIntervalSince1970: (timestamp ?? 0)/1000)
    ...
    func durationStringForChat(duration: String) -> String {
        let fromDate:Date = Date(timeIntervalSince1970: ((duration.toDouble ?? 0.0)))
        let fromDateFormat = fromDate.toDate(format: "dd MM yyyy")
        let todayDateFormat = Date().toDate(format: "dd MM yyyy")
        var durations = "CJustNow"
        
        if (fromDateFormat == todayDateFormat)  {
            durations = DateFormatter.dateStringFrom(timestamp: (duration.toDouble ?? 0.0), withFormate: kHHmm)
        } else {
            durations = DateFormatter.dateStringFrom(timestamp: (duration.toDouble ?? 0.0), withFormate: kddMMMYYYhhmm)
        }
        
        return durations;
    }
    
    func durationDoubleForChat(duration: Double) -> String {
        let calender:Calendar = Calendar.current as Calendar
        let fromDate:Date = Date(timeIntervalSince1970: duration/1000 )
        ...
        var durations: String = "CJustNow"
    
        if (days > 0) {
            durations = DateFormatter.dateStringFrom(timestamp: duration, withFormate: kddMMYYYY)
        } else {
            durations = DateFormatter.dateStringFrom(timestamp: duration, withFormate: kHHmm)
        }
